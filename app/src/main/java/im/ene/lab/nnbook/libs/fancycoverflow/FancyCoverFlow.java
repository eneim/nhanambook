...
package im.ene.lab.nnbook.libs.fancycoverflow;
...
public class FancyCoverFlow extends Gallery {
	...
	protected boolean getChildStaticTransformation(View child,
		...
		final FancyCoverFlowItemWrapper item = (FancyCoverFlowItemWrapper) child;
		...
		final int coverFlowCenter = (int) (coverFlowWidth / 2);
		...
		// alpha).
		transform.clear();
		transform.setTransformationType(Transformation.TYPE_BOTH);

		// Alpha
		if (this.unselectedAlpha != 1) {
			final float alphaAmount = (this.unselectedAlpha - 1)
					* Math.abs(effectsAmount) + 1;
			transform.setAlpha(alphaAmount);
		}
		...
		final Matrix imageMatrix = transform.getMatrix();
